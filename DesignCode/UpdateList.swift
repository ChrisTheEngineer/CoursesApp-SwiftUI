...
struct UpdateList: View {
    @ObservedObject var store = UpdateStore() //needed to be able to modify dynamic data
    
    func addUpdate() {
        store.updates.append(Update(image: "Card1", title: "NewItem", text: "text", date: "Jan 1"))
    }
    
    var body: some View {
        NavigationView {
            List { //make it a list naviation view
                ForEach(store.updates) { update in //must use a foreach if want swipe delete functionality
                ...
                .onDelete { index in
                    self.store.updates.remove(at: index.first!) //! forces value to not be optional
                }
            ...
            .navigationBarTitle(Text("Updates"))
                .navigationBarItems(leading: Button(action: addUpdate) { //curly braces in button not needed if using a func
                    Text("Add Update")
                }, trailing: EditButton())
        }
    }
